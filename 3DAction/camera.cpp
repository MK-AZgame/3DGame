...
void UpdataCamera(void)
{
	...
	//	////カメラの回り込み
	//	//目的の角度の設定
	//	g_camera.rotDest.y = -sinf(pPlayer->rot.y);

	//	//目的の位置
	//	g_camera.posVDest.x = pPlayer->pos.x - sinf(g_camera.rot.y) * 100.0f;
	//	g_camera.posVDest.y = pPlayer->pos.y + 100.0f;
	//	g_camera.posVDest.z = pPlayer->pos.z - cosf(g_camera.rot.y) * 100.0f;

	//	//現在の角度の計算
	//	g_camera.rot += (g_camera.rotDest - g_camera.rot) * 0.1f;
	//		
	//	//
	//	g_camera.posV += (g_camera.posVDest - g_camera.posV) * 0.1f;

	//	//D3DXVec3Normalize(&g_camera.rot, &g_camera.rot);
	//}
	//else
	{
		//カメラの追従処理
		g_camera.posRDest.x = pPlayer->pos.x + sinf(g_camera.rot.y) * 100.0f;
		g_camera.posRDest.y = pPlayer->pos.y;
		g_camera.posRDest.z = pPlayer->pos.z + cosf(g_camera.rot.y) * 100.0f;

		g_camera.posVDest.x = pPlayer->pos.x - sinf(pPlayer->rot.y) * 10.0f;
		g_camera.posVDest.y = pPlayer->pos.y;
		g_camera.posVDest.z = pPlayer->pos.z - cosf(pPlayer->rot.y) * 10.0f;

		g_camera.posR += (g_camera.posRDest - g_camera.posR) * 0.1f;
		g_camera.posV += (g_camera.posVDest - g_camera.posV) * 0.1f;
		g_camera.rot += (g_camera.rotDest - g_camera.rot) * 0.1f;

		//g_nCntTime = 0;						//待機時間を0に戻す
	}
